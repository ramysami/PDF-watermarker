...
for input_name in input_names_list:
    ...
    for i, watermark_name in watermarks_names_list and enumerate(watermarks_names_list, start=1):
        print('Adding watermark to ' + clean_name + ' ' + watermark_name)
        create_watermark(
            input_pdf=f'input/{input_name}',
            output=f'output/{i} - {clean_name} {watermark_name}',
            watermark=f'watermarks/{watermark_name}'
        )
print("Done")
